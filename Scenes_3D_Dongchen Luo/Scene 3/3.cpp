...
void Draw_vector_field()
{
    ...
    for (double t = -2.0; t <= 1.0; t += 0.3)
    {
        double c1 = pi / 4 + t;
        double c2 = 2 + t;


        float x_base = cos(c1) * c2;
        float y_base = sin(c1) * c2;
        float z_base = c2;


        float x_vector = cos(c1);
        float y_vector = sin(c1);
        float z_vector = -1.0;


        glBegin(GL_LINES);
        glVertex3f(x_base, y_base, z_base);
        glVertex3f(x_base + x_vector, y_base + y_vector, z_base + z_vector);
        glEnd();
